...
下のタブ（お買い物・よく買うもの・支出・家計・ストック・設定）は、ボタンをタップする以外に
...
下部タブの「よく買う」、または**お買い物タブのフローティングボタン（⚡よく買う）**から開けます
（下部タブに気づきにくいという声を受けて、お買い物ページからも同じ内容をシートで開けるようにしました）。
定番品を登録しておくと、**カードをタップして確認するだけでリストに追加**できます（項目数が増えて
隣のカードに触れてしまいやすいため、確認をひとつ挟みます）。カテゴリごとに並びます。

**表示は「🖼️ カード」「☰ リスト」の2種類から選べます。** カードは写真中心で見やすく、
リストは品名を詰めて一度にたくさん見渡せます。切り替えは端末ごとに覚え、
お買い物ページから開くシートでも共通です。
...
**登録の仕方は2つ**
1. 「よく買う」タブを開いた状態で **＋（画面右下）**から入力する（他のタブと同じ導線。写真も付けられます）
...
下部タブの「支出」から開きます。完了件数と支出（💴を記録していれば実支出、無ければ予算合計）が見られます。
...
- **💴 金額** — 実際に払った金額を記録できます。記録すると「💴 支出・家計」タブの集計が**実支出の集計**に変わります
...
**📊 今月のおかいもの**は設定タブではなく、**「💴 支出・家計」タブ**にあります（→ [6章](#6--支出家計)）。
