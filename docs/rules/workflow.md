...
| `icon-picker-test` | 🎨 イラストから選ぶピッカー（おつかい/よく買うもの/ストックの写真欄で共通・12分類をタブで切り替え・検索は分類をまたいで探す） |
...
| `category-required-test` | カテゴリ選択の必須化（未選択だとブロック・📎未分類の明示選択は廃止・編集時の選択済み表示） |
| `badge-seen-test` | お買い物タブの赤バッジ（既読で消える・新着依頼だけ数える・見た依頼は再表示されない） |
| `empty-state-test` | お買い物リストが空のときの案内（キャラクター＋吹き出し・はじめかたの3ステップ） |
| `add-sheet-fold-test` | おつかい追加シートの任意項目の折りたたみ（開いた直後は品名・カテゴリ・写真・追加ボタンだけ・小画面で追加ボタンが画面内・「設定あり」の要約・編集時の自動展開） |
| `stock-sheet-fold-test` | ストック登録シートの任意項目の折りたたみ（開いた直後は商品名・カテゴリ・写真・残量・追加ボタンだけ・小画面で追加ボタンが画面内・「設定あり」の要約） |
| `profile-emoji-test` | プロフィールの絵文字ピッカー（表情/どうぶつ/ひと/のりものの4タブ切り替え・選んでいる絵文字の分類が自動で開く・保存中の再描画で選択が巻き戻らないこと） |
| `continue-add-test` | 🔁 続けて追加する（追加モードだけに出る・ONで閉じずに次の入力へ・カテゴリ/行き先は残る・OFFなら従来どおり閉じる） |
